...
const InscriptionsTab = ({ tournamentId, tournamentStatus }) => {
  ...
  const unregisterMutation = useMutation({
    ...
    onSuccess: () => { invalidateRegs(); toastSuccess('Compétiteur désinscrit'); },
  ...
  const setSeedMutation = useMutation({
    ...
    onSuccess: () => { invalidateRegs(); toastSuccess('Classement mis à jour'); },
  ...
  const setCategoryMutation = useMutation({
    ...
    onSuccess: () => { invalidateRegs(); toastSuccess('Catégorie assignée'); },
                ...
                return (
                  <tr key={reg.id} className="bg-background hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {canUnregister && (
                        <ConfirmActionDialog
                          trigger={<Button size="sm" variant="destructive" disabled={unregisterMutation.isPending}>Désinscrire</Button>}
